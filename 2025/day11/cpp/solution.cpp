...
uint64_t part2(const Graph& graph) {
    ...
    auto count_to_target = [&graph](const string& start, const string& target) -> uint64_t {
        Memo memo;
    ...
    uint64_t svr_to_dac = count_to_target("svr", "dac");
    uint64_t dac_to_fft = count_to_target("dac", "fft");
    uint64_t fft_to_out = count_to_target("fft", "out");

    // Count paths: svr -> fft -> dac -> out
    uint64_t svr_to_fft = count_to_target("svr", "fft");
    uint64_t fft_to_dac = count_to_target("fft", "dac");
    uint64_t dac_to_out = count_to_target("dac", "out");
